    ...
    <div class="rounded-lg border border-slate-700 bg-slate-900 p-5 shadow-lg shadow-black/20">
      <div class="mb-4 grid grid-cols-2 gap-2">
        <button
          ...
          :class="!isRegister ? 'border-emerald-400 bg-emerald-500/15 text-emerald-100' : 'border-slate-700 bg-slate-800 text-slate-300'"
        >
          登录
        </button>
        <button
      ...
      <form class="space-y-4" @submit.prevent="submit">
        <label class="block">
          ...
          <input
            ...
            class="h-10 w-full rounded-lg border border-gray-600 bg-gray-800 px-3 text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500"
          />
        </label>
        ...
        <div v-if="isRegister" class="flex flex-wrap gap-2">
          <button
            v-for="uid in demoUsers"
            :key="uid"
            type="button"
            @click="recipeUserId = uid"
            class="rounded px-2 py-1 text-xs transition-colors"
            :class="recipeUserId === uid ? 'bg-primary-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'"
          >
            {{ uid }}
          </button>
        </div>
        ...
        <button
          type="submit"
          :disabled="loading"
          class="h-10 w-full rounded-lg bg-primary-600 font-medium text-white transition-colors hover:bg-primary-500 disabled:cursor-not-allowed disabled:bg-gray-600"
        >
          {{ loading ? "请稍候..." : isRegister ? "创建并登录" : "登录" }}
        </button>
      </form>

      <div v-if="error" class="mt-4 rounded border border-red-500/30 bg-red-950/30 px-3 py-2 text-sm text-red-200">
        {{ error }}
      </div>
    </div>
  </div>
  ...
  try {
    ...
    const { data } = isRegister.value
      ? await registerUser({
          ...
          recipe_user_id: recipeUserId.value,
        })
